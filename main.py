...
async def setup_logging(config: Config):
    ...
    if config.log_tg_cert_enabled:
        if 'telegram_cert' in handlers:
            ...
            handlers['telegram_cert']['thread_id'] = (
                config.log_tg_cert_thread_id
            )
    else:
    ...
    if config.log_error_tg_enabled:
        if 'telegram_error' in handlers:
            ...
            handlers['telegram_error']['thread_id'] = (
                config.log_error_tg_thread_id
            )
    else:
    ...
    for logger_cfg in log_config.get('loggers', {}).values():
        if 'handlers' in logger_cfg:
            logger_cfg['handlers'] = [
                h for h in logger_cfg['handlers'] if h in handlers
            ]

    # То же самое для root
    if 'handlers' in log_config.get('root', {}):
        log_config['root']['handlers'] = [
            h for h in log_config['root']['handlers'] if h in handlers
        ]
...
async def setup_redis(
    config: Config,
) -> tuple[Redis, RedisStorage, Redis]:
    ...
    redis_data = Redis(
        host=config.redis_host, port=6379, db=2, decode_responses=True
    )
