    ...
    async def __call__(
        ...
        if check_user and int(check_user.decode()) == 1:
            if isinstance(event, Message):
                ...
                asyncio.create_task(
                    msg_processor.deletes_msg_a_delay(value, 5)
                )
            ...
            await self.storage.redis.set(
                name=throttl_user_id, value=2, px=5000
            )
        ...
        if not check_user:
            await self.storage.redis.set(
                name=throttl_user_id, value=1, px=self.ttl
            )
...
class MsgProcMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        Args:
            ...
            data: Словарь с данными, которые передаются между middleware
             и обработчиками.
        Returns:
            handler.
        """
...
class StrictMaintenanceMiddleware(BaseMiddleware):
    ...
    def __init__(
        ...
        redis: Redis,
        enabled: bool = False,
        message: str = '⚙️ Идут технические работы. '
        ...
        try:
            if getattr(event, 'message', None):
                ...
                await event.callback_query.answer(
                    self.message, show_alert=True
                )
            else:
                # универсальный fallback для прочих апдейтов
                bot = data.get('bot')
                user = data.get('event_from_user')
                if bot and user:
